...
<div class="lang-cn-block">
<ul class="space-y-3 text-slate-400 text-sm">
<li><code class="text-primary">plotly::plotlyOutput()</code> — 渲染交互式时间序列折线图（模块1）、按聚类颜色编码的PCA散点图（模块2）、肘部图和轮廓诊断图（模块2）、变量重要性柱状图和交叉验证 &lambda; vs MSE 曲线（模块3）。Plotly 在所有图表中提供悬停提示、缩放和刷选功能。</li>
<li><code class="text-primary">leaflet::leafletOutput()</code> — 渲染模块1中的哥伦比亚省份分级统计地图，按用户选择的指标（客户数量、平均流失概率或平均CLV）着色。用户可以缩放、平移和点击省份查看详细提示。</li>
<li><code class="text-primary">DT::DTOutput()</code> — 显示模块2中的聚类汇总统计表（每个聚类各特征的均值）和模块3中的当前与假设对比表。两个表格均支持排序、搜索和分页。</li>
<li><code class="text-primary">uiOutput()</code> — 用于模块1中的统计检验结果面板（动态渲染检验统计量、p值、效应量和解释）和模块3中的流失概率仪表（随 What-If 滑块变化而更新的响应式HTML仪表）。</li>
</ul>
</div>
</div>
